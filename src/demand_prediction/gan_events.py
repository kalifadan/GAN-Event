...
EMBEDDINGS_TRAIN_GAN_PATH = "../gan_embeddings/" + RESULTS_PREFIX + "causal_mean_train_gan_embeddings.pkl"
EMBEDDINGS_TEST_GAN_PATH = "../gan_embeddings/" + RESULTS_PREFIX + "causal_mean_test_gan_embeddings.pkl"


def create_embeddings_df(embeddings_df, train_df, test_df, peaks_data, limit_size=1):
    gen_data = peaks_data.merge(embeddings_df, on='date', how='left')
    ...
    X_gen = gen_data.drop(columns=['peak', 'Quantity'])
...
def get_gan_embeddings(train_df, test_df, peaks_data, window_size=2):
    ...
    train_set, test_set = create_embeddings_df(embeddings_df, train_df, test_df, peaks_data, limit_size=(window_size-1))
    return train_set, test_set
